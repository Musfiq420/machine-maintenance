...
const MachineDetails = () => {
  ...
  const chartData = breakdowns.map((breakdown) => {
    ...
    const lostTimeInMinutes = parseInt(lostTimeParts[0]) * 60 + parseInt(lostTimeParts[1]);
    const breakdownDate = new Date(breakdown["breakdown-start"]).toLocaleDateString();
    return {
      date: breakdownDate,
  ...
  return (
    <div className="bg-green-50 p-6 rounded-lg shadow-lg">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-green-800">Machine {machineData.machine_id}</h1>
          <h2 className="text-lg text-green-700">Model: {machineData.model_number}</h2>
          <p className="text-gray-500">
            Serial No: <span className="text-gray-700 font-medium">{machineData.serial_no}</span>
          </p>
        </div>
        <div className="w-2/3">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="text-center bg-white p-4 rounded-lg shadow">
          ...
          <h3 className="text-xl font-bold text-gray-800">{machineData["total-lost-time-last-week"]}</h3>
        </div>
        <div className="text-center bg-white p-4 rounded-lg shadow">
          <p className="text-sm text-gray-500">Utilization (Last Week)</p>
          <h3 className="text-xl font-bold text-gray-800">{(machineData["utilization-last-week"] * 100).toFixed(1)}%</h3>
        </div>
        <div className="text-center bg-white p-4 rounded-lg shadow">
          <p className="text-sm text-gray-500">Breakdown Count (Last Week)</p>
          <h3 className="text-xl font-bold text-gray-800">{machineData["breakdowns-count-last-week"]}</h3>
        </div>
        <div className="text-center bg-white p-4 rounded-lg shadow">
          <p className="text-sm text-gray-500">MTBF (Last Week)</p>
          <h3 className="text-xl font-bold text-gray-800">{machineData["MTBF-last-week"]}</h3>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow">
          <h3 className="text-lg font-semibold text-gray-700 mb-4">Breakdown Timeline (Last Week)</h3>
          <BarChart
            width={500}
            height={300}
            data={chartData}
          ...
          >
            ...
            <YAxis label={{ value: 'Lost Time (mins)', angle: -90, position: 'insideLeft' }} />
            <Tooltip />
            <Legend />
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          <h3 className="text-lg font-semibold text-gray-700 mb-4">Lost Time Reasons (Last Week)</h3>
